// zadanie_stl.cpp : Defines the entry point for the console application.
//
...
uniform_real_distribution<float> dist2(1, 999);
...
public:
	...
	void odleglosc() {
		...
		try {
			...
			cout << "Odleglosc z miasta " << nazwa1 << " do miasta " << nazwa2 << " jest rowna: " << pos_miasta1.odl(pos_miasta2) << "km." << endl;
		}
		catch (out_of_range) {
...
int main()
{
	...
	matrix1.wyswietl();
	Dzien d1;
	d1.dodaj_zadanie(10, "zadanie 1");
	d1.dodaj_zadanie(15, "zadanie 2");
	d1.dodaj_zadanie(22, "zadanie 3");
	d1.dodaj_zadanie(8, "zadanie 4");
	d1.dodaj_zadanie(3, "zadanie 5");
	cout << "----------------------" << endl;
	d1.wyswietl();
	d1.wykonaj();
	d1.wyswietl();
	...
	cout << "----------------------" << endl;
	Miasta mapa_miast1;
	mapa_miast1.dodaj_miasto("Gliwice", dist2(eng), dist2(eng));
	mapa_miast1.dodaj_miasto("Rybnik", dist2(eng), dist2(eng));
	mapa_miast1.dodaj_miasto("Katowice", dist2(eng), dist2(eng));
	mapa_miast1.dodaj_miasto("Zabrze", dist2(eng), dist2(eng));
	mapa_miast1.dodaj_miasto("Tristram", dist2(eng), dist2(eng));
	mapa_miast1.odleglosc();
